#!/usr/bin/env bash
# End-to-end smoke test for a running github-mcp HTTP server.
#   ./deploy/smoke-test.sh [http_base] [metrics_base]
# Exercises /healthz, /readyz, an MCP session (initialize -> initialized ->
# tools/list -> tools/call get_repo), and asserts the metrics counter
# incremented. Requires curl + jq. Uses only free read tools — no token needed.
set -euo pipefail
...
EXPECTED="add_issue_comment create_issue get_repo list_issues search_repositories "
[ "$TOOLS" = "$EXPECTED" ] && pass "5 tools listed" || fail "unexpected tools: [$TOOLS]"

echo "== tools/call get_repo =="
CALL=$(curl -sf -L -H "Content-Type: application/json" -H "Accept: ${ACCEPT}" -H "mcp-session-id: ${SESSION}" \
  -X POST "$MCP_URL" -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"get_repo","arguments":{"owner":"python","repo":"cpython"}}}' | sse_data)
[ "$(printf '%s' "$CALL" | jq -r '.result.isError')" = "false" ] && pass "get_repo ok" || fail "call errored: $CALL"

echo "== metrics =="
if curl -sf "${METRICS_BASE}/metrics" | grep '^mcp_tool_calls_total{' | grep 'tool="get_repo"' | grep -q 'status="success"'; then
  pass "mcp_tool_calls_total incremented for get_repo"
else
  fail "success metric not found for get_repo"
fi
